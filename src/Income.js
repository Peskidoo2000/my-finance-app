...
const storedData = JSON.parse(localStorage.getItem("storedIncomeData")) || [];
...
function storeData(){
if(incomedate.trim()!=="" && amount.trim()!==""){
const storedData = JSON.parse(localStorage.getItem("storedIncomeData")) || [];
storedData.push(incomeEntry);
localStorage.setItem("storedIncomeData", JSON.stringify(storedData)); 
setIncomedate("");
setAmount("")
setIncome("")
console.log( "stored-Data", storedData);
}else{
  incomedate.length === 0 ? alert("Enter a date") :  alert("Enter an Amount");

}

}  
    
    const handleIncomeDate =(event)=>{
      ...
      if(filteredData.length === 0){
        setError("No Income data Entry for the selected widget");
        window.myChartInstance.destroy();
        }else{
  ...
  const handleSelect = (event)=>{
    ...
    return(
        <div className="dashboard-parent">
          ...
          {(dateinput !== "") &&      
           ...
           <div className="first-row">
             ...
             <table className="table">
                ...
                <td className="td"> Income Type</td>
          ...
          <Button type="button" onClick={showAddincome} target={addincome} variant="outlined" className="add-income">Add income</Button>
          </div>
         
          {(addincome) && <div>
            <form className="income-form">
                <label htmlFor="options" className="form-heading income"> Type</label><br/>
                <select onChange={handleAddincome} value={income} className="select-income">
                    <option value=""></option>
                ...
                {(income) && <div className="amount">
                    ...
                    <input type="date" className="income-date" onChange={handleIncomeDate} value={incomedate}/>
                    </div>

                    
                    <Button variant="contained" className="save-income" onClick={storeData}>Save Income</Button>
                     </div>} 
            </form>                  
          </div>
}
